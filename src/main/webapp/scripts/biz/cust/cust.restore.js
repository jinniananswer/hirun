var custRestore = {
    ...
    queryCustList4Cond : function(obj) {
        ...
        custRestore.queryCustList(param);
        hidePopup(obj);
    },
    ...
    restoreCust : function(custId) {
        MessageBox.success("提示信息","确定恢复吗？", function(btn){
            if("ok" == btn) {
                $.beginPageLoading('客户恢复中');
                $.ajaxReq({
                    url : 'cust/restoreCustById',
                    data : {
                        CUST_ID : custId
                    },
                    type : 'POST',
